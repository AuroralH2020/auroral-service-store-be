...
module.exports = async function () {
  ...
  try {
    ...
    eventsService.emit('models loaded');
    ...
    if (isMultitenantEnabled) {
      ...
      if (preloadData || savedTenants.length === 0) {
        try {
          ...
          await tenantsService.bulkCreate(tenants, { ignoreSerialPk: false, updateAssociations: false, ignoreIds: false, syncModels: false });
        } catch (error) {
    ...
    if (preloadData) {
      ...
      if (fs.existsSync('./data/data.config.json')) {
        ...
        console.log('Data files to process:', dataFilenames.join(', '));
      }
      ...
      for (const [index, filenames] of dataFileGroups.entries()) {
        ...
        for (const data of resourcesData) {
          ...
          if (isMultitenantEnabled) {
            const tenantName = filename.split('.')[0];
            ...
            resourceService.changeTenant(tenantName);
            console.log(i18n.__('Changed to tenant \'{{tenantName}}\'', { tenantName }));
          }
          ...
          if (
            ...
            try {
              ...
              for (const resource of resources) {
                const errors = await resourceService.validate(resource);
                if (Array.isArray(errors)) {
                  const resourceId = resource[resourceModel.primaryKeyField] || resource[Object.keys(resource)[0]];
                  throw new Error(errors
                    .map(error => `Error creating '${resourceId}' in '${resourceName}': ` +
                      i18n.__(error.message, { field: error.field, args: error.args }))
                    .join('. '));
                }
              }
              await resourceService.bulkCreate(resources, { ignoreSerialPk: false, updateAssociations: false });
              console.log(`Success creating '${resourceName}' in bulk`);
            } catch (error) {
              console.error(error.message);
              errors.push({ action: 'bulk create', resource: resourceName, message: error.message });
              continue;
            }
          }
          ...
          // Then, create resources one by one to also create associated resources at the same time
            ...
            for (resource of resources) {
              try {
                ...
                if (Array.isArray(errors)) {
                  throw new Error(errors
                    .map(error => i18n.__(error.message, { field: error.field, args: error.args }))
                    .join('. '));
                }
                await resourceService.create(resource, { ignoreSerialPk: false, createAssociatedEntities: true, updateAssociations: false });
                const resourceId = resource[resourceModel.primaryKeyField] || resource[Object.keys(resource)[0]];
        ...
        if (index === 0 && isRuntimeResourcesEnabled) {
          // Register event listener for resources controller events
          // and create layers for runtime resources
          await require('@loaders/resources.loader')();
        }
      }

      if (errors.length === 0) {
        console.log('Data successfully loaded with no errors');
      } else {
        console.log(`Data loaded with ${errors.length} errors:`, errors);
      }
