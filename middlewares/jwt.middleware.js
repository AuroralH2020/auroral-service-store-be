...
const { adminFolder, isApiSecured, routesWhitelist, isMultitenantEnabled } = require('@config');
...
module.exports = async function (req, res, next) {
  try {
    // If API endpoints authentication is disabled or requested path is whitelisted, skip to next middleware
    if (!isApiSecured ||
      routesWhitelist.some(path => path.includes('@')
        // If path includes @, it includes which methods are whitelisted for this path
        ? path.split('@')[0].split('|').some(method => method === req.method) &&
        // If path starts with REGEXP, it is a regular expression instead a plain string
        (path.split('@')[1].startsWith('REGEXP')
          ? new RegExp(path.split('@')[1].slice(6)).test(req.path)
          : req.path.startsWith(path.split('@')[1]))
        : path.startsWith('REGEXP')
          ? new RegExp(path.slice(6)).test(req.path)
      ...
      if (isMultitenantEnabled) {
        ...
        if (req.get('Authorization') && req.get('Authorization').split(' ')[0] === 'Bearer') {
          ...
          const payload = oldToken ? jwt.verifyToken(oldToken) : undefined;
          if (payload) {
            const queryObj = {};
      ...
      if (!req.get('Authorization')) throw httpErrors.create(401, res.__('Token not provided'));
      const [type, oldToken] = req.get('Authorization').split(' ');
      if (type !== 'Bearer') throw httpErrors.create(401, res.__('Invalid authorization type'));
      if (!oldToken) throw httpErrors.create(401, res.__('Token not provided'));
